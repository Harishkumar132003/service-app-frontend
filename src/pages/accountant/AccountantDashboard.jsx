...
import { Box, Button, Card, CardContent, Container, Stack, Typography, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material'
...
export default function AccountantDashboard() {
	...
	function openConfirm(t){ setSelected(t); setConfirmOpen(true) }
	function closeConfirm(){ setConfirmOpen(false); setSelected(null) }
	...
	return (
		<Box>
						...
						<Card key={t.id}><CardContent>
							...
							<Stack direction="row" spacing={1} sx={{ mt: 1, alignItems: 'center' }}>
								<Button variant="outlined" component="label" size="small">
									{paymentFileMap[t.id]?.name || 'Upload Payment Image (optional)'}
									<input type="file" hidden accept="image/*" onChange={(e)=>{
										const f = e.target.files?.[0]
										setPaymentFileMap(v=>({ ...v, [t.id]: f }))
									}} />
								</Button>
								<Button onClick={()=>openConfirm(t)}>Mark As Paid</Button>
							</Stack>
						</CardContent></Card>
					))}
				</Stack>
			...
			<Dialog open={confirmOpen} onClose={closeConfirm}>
				...
				<DialogContent>
					Mark this invoice as paid? {paymentFileMap[selected?.id]?.name ? '(with image)' : ''}
				</DialogContent>
				<DialogActions>
					<Button onClick={closeConfirm}>Cancel</Button>
					<Button onClick={onPaid} variant="contained">Confirm</Button>
				</DialogActions>
			</Dialog>
		</Box>
	)
